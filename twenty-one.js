...
class TwentyOneGame {
  ...
  playAgain() {
    ...
    while(true) {
      answer = readline.question("Do you want to play again? (y/n)").toLowerCase();
  ...
  dealerTurn() {
    ...
    while(true) {
      ...
      if (score >= TwentyOneGame.DEALER_MAX) break;
      this.dealerContinue();
  ...
  computeScoreFor(hand) {
    ...
    cards.filter((card) => card.isAce() && !card.isHidden()).forEach(() => {
